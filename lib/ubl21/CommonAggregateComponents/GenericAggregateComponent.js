 ...
 */
class GenericAggregateComponent {
    /**
     *
     ...
     */
    constructor(content, paramsMap, name = 'GenericAggregateComponent') {
    ...
    parseToJson() {
        ...
        Object.keys(this.paramsMap)
            ...
            jsonResponse[attributeName] = Array.isArray(this.attributes[attKey])
                ? this.attributes[attKey].map((e) => e.parseToJson())
    ...
    assignContent(content) {
        ...
        Object.keys(content || {})
            ...
            if (Array.isArray(content[att])) {
                this.attributes[att] = content[att].map((subItem) => {
